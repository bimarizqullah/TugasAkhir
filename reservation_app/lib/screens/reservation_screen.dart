...
class _ReservationScreenState extends State<ReservationScreen> {
  ...
  List<dynamic> _bookedSlots = []; // slot yang sudah terpakai
  ...
  Future<void> _pickDate() async {
    ...
    // Load slot terpakai setelah tanggal & meja dipilih
  ...
  Future<void> _pickStartTime() async {
    ...
    setState(() {
      _startTime = t;
      // Reset end time jika sekarang lebih awal dari start
  ...
  Future<void> _pickEndTime() async {
    ...
    // Cek apakah rentang waktu bentrok dengan slot terpakai
    if (_isConflict(t)) {
  ...
  /// Cek bentrok: apakah [_startTime, endTime] overlap dengan slot yang terpakai
  bool _isConflict(TimeOfDay endTime) {
    ...
    for (final slot in _bookedSlots) {
      final parts1 = (slot['start_time'] as String).split(':');
      final parts2 = (slot['end_time']   as String).split(':');
      final sStart = int.parse(parts1[0]) * 60 + int.parse(parts1[1]);
      final sEnd   = int.parse(parts2[0]) * 60 + int.parse(parts2[1]);
    ...
    try {
      ...
      if (result['status'] == 201) {
        ...
        if (code == 'TIME_CONFLICT') {
          ...
          // Reload slot agar tampilan terupdate
          await _loadBookedSlots();
        } else {
          _showSnackBar(msg);
        }
            ...
            child: Text(
              ...
              style: TextStyle(
                ...
                color     : hasValue
                    ? _textDark
                    : (isActive
                        ? _textGrey.withValues(alpha: 0.7)
                        : _borderColor),
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<int>(
          ...
          hint      : Text('Pilih meja',
              style: TextStyle(
                  color: _textGrey.withValues(alpha: 0.7), fontSize: 13)),
  ...
  Widget _buildTextField({
    ...
    return TextFormField(
      ...
      decoration     : InputDecoration(
        hintText  : hint,
        hintStyle : TextStyle(
            color: _textGrey.withValues(alpha: 0.6), fontSize: 13),
